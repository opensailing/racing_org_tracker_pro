defmodule NauticNet.SecureTransport.RegisterClient do
  @moduledoc """
  ...
  `SailRoute.Devices.register_pop_message/2`:

      "SailRoute-DeviceRegister-v1"
        || lp(public_key_raw_32)
        || lp(Integer.to_string(timestamp))

  where:

    * the domain `"SailRoute-DeviceRegister-v1"` is the LITERAL ASCII bytes — it is
      NOT length-prefixed;
  ...
  Options:
    ...
    * `:boat_identifier` — the optional boat identifier (default the device hostname
      via `NauticNet.boat_identifier/0`).
