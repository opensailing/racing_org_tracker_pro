defmodule NauticNet.SecureTransport.KeyStore do
  @moduledoc """
  ...
  `SailRoute.Devices.DeviceKey`. This module owns the device's STABLE PRIVATE
  ...
  configurable via
      ...
      config :nautic_net_device, #{inspect(__MODULE__)}, base_path: "/some/dir"
  ...
  chars) — IDENTICAL to the server's `SailRoute.Devices.DeviceKey.fingerprint/1`.
  ...
  IDENTICAL to the server's `SailRoute.Devices.DeviceKey.fingerprint/1`. Uses
