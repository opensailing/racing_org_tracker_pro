defmodule NauticNet.SecureTransport.ChannelHandler do
  @moduledoc """
  PURE protocol logic for the secure-transport command channel, extracted from the
  Slipstream transport (`NauticNet.SecureTransport.ChannelClient`) so it can be
  ...
  Three concerns, all side-effect-light and deterministic given their inputs:
    ...
    * `handle_command/3` — given a server `"command"` payload, decode the
      `ServerReply` protobuf, apply it through `NauticNet.Commands` (the SAME
  ...
  applying it through `NauticNet.Commands` (the shared, idempotent command path).

  `command_server` is the `NauticNet.Commands` GenServer name/pid (defaults to
  `NauticNet.Commands`).
  ...
  def handle_command(payload, command_id, command_server \\ Commands) do
    with {:ok, reply_b64} <- fetch_field(payload, "reply"),
         ...
         {:ok, %struct{} = reply} when struct == NauticNet.Protobuf.ServerReply <-
