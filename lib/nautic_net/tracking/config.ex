defmodule NauticNet.Tracking.Config do
  @moduledoc """
  ...
  Mirrors `NauticNet.WiFiManager` exactly:

    * The persisted config in `NauticNet.Tracking.Store` is the AUTHORITY. On boot it
    ...
    * `apply_config/2` is idempotent on `version`: a `version` already applied (`<=`
      ...
      side effect (which re-drives `NauticNet.Sampling`), and returns `{:ok, config}`.
