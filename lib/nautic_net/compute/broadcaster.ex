defmodule NauticNet.Compute.Broadcaster do
  @moduledoc """
  ...
  `broadcast_rate_hz` (Phase 8). It mirrors `NauticNet.Nav.Broadcaster`: a GenServer
  ...
  On each tick it reads `NauticNet.Compute.Engine.current_values/1` and, for every def
  that is
  ...
  it DAMPS the output(s) then ENCODEs (`NauticNet.Compute.PgnEncode`) and SENDs. The
  ...
  Per def, an EWMA (`NauticNet.Telemetry.Ewma`, the Phase 5 time-constant filter) is
