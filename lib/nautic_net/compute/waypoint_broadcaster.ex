defmodule NauticNet.Compute.WaypointBroadcaster do
  @moduledoc """
  ...
  When the device holds an active race assignment (`NauticNet.Commands`) whose course
  ...
  See `NauticNet.Compute.PgnEncode.navigation_data_129284/1` and `route_wp_129285/1`
  for the wire layouts.

  It mirrors `NauticNet.Compute.RaceTimerBroadcaster`: a GenServer ticking on a timer,
  ...
  the destination lat/lon. This is the SAME derivation `NauticNet.Nav.State` uses. The
  ...
  # --- destination derivation (mirrors NauticNet.Nav.State) -------------------------
