defmodule NauticNet.FirmwareValidator do
  @moduledoc """
  Marks the running firmware VALID once the device has connected to the SailRoute
  ...
  successful SailRoute connection, NOT merely on boot or NervesHub:

    * a good OTA reaches SailRoute → validates itself → STICKS.
    * a bad OTA that cannot establish the SailRoute session → never validates →
  ...
  def validate_on_connect(opts \\ []) do
    ...
    cond do
      ...
      true ->
        ...
        Logger.info("[FirmwareValidator] firmware validated after SailRoute connect")
        :validated
    end
  rescue
    e ->
