defmodule NauticNet.Sampling do
  ...
  Re-evaluates the race phase from the active assignment (`NauticNet.Commands`), the
  ...
  `{damping_seconds, send_rate_hz}` from `NauticNet.Tracking.Config` (the
  server-pushed, persisted config), then sets BOTH the
  `NauticNet.Telemetry.Reporter` flush interval (`round(1000 / send_rate_hz)`) AND the
  Reporter damping τ.
  ...
  Re-evaluation is triggered on the periodic tick, on a `NauticNet.Commands` change,
  and on a `NauticNet.Tracking.Config` change (via `reconfigure/1`, wired as the
  Config's `on_apply`).
  """
  use GenServer
  ...
  changes). Returns `:ok`. Wired as `NauticNet.Tracking.Config`'s `on_apply`.
  """
