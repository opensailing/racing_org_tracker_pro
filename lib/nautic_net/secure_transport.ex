defmodule NauticNet.SecureTransport do
  @moduledoc """
  NauticNet Secure Transport (v2) — a self-contained, transport-agnostic
  cryptographic handshake + authenticated-encryption framing library.

  DEVICE-side port of the SailRoute Secure Transport crypto core. This implementation
  is **byte-for-byte compatible** with the server's `SailRoute.SecureTransport.*`
  ...
  **not** touch the database, channels, UDP, or protobuf; it operates only on binaries.
  ...
  The implementation lives in:
    ...
    * `NauticNet.SecureTransport.Primitives` — `:crypto` wrappers (Ed25519, X25519,
      ChaCha20-Poly1305 with the mandatory structural tag/nonce checks), X25519 point
      validation, constant-time compare.
    * `NauticNet.SecureTransport.HKDF` — RFC 5869 HKDF over HMAC-SHA256.
    * `NauticNet.SecureTransport.Handshake` — initiator + responder roles.
    * `NauticNet.SecureTransport.Session` — established session state.
    * `NauticNet.SecureTransport.Frame` — wire framing, seal/open.
    * `NauticNet.SecureTransport.ReplayWindow` — per-(session,epoch) replay defence.
  ...
  @hkdf_label "SailRoute-SecureTransport-v1"
  ...
  alias NauticNet.SecureTransport.{Frame, Handshake, Primitives}
