defmodule RacingOrg.Tracker.Pro.Nav.PGN do
  @moduledoc """
  Encodes outbound NMEA 2000 navigation *display* PGNs from a `RacingOrg.Tracker.Pro.Nav.State`:
  ...
  def navigation_data_129284(%State{} = state, sid \\ 0) do
    ...
    <<sid::8, encode_u32(state.distance_to_dest_m, 100)::little-32, flags::8, @u32_unknown::little-32,
      @u16_unknown::little-16, encode_rad(state.bearing_origin_to_dest_rad)::little-16,
      encode_rad(state.bearing_position_to_dest_rad)::little-16, encode_wp(state.origin_wp_number)::little-32,
      encode_wp(state.destination_wp_number)::little-32, dest_lat::little-signed-32, dest_lon::little-signed-32,
      @s16_unknown::little-signed-16>>
  end
  ...
  best-effort encoding. Prefer the GPX export (`RacingOrg.Tracker.Pro.Nav.GPX`) when route
