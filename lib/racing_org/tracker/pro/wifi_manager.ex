defmodule RacingOrg.Tracker.Pro.WiFiManager do
  ...
  The persisted state in `RacingOrg.Tracker.Pro.WiFi.Store` is the AUTHORITY. On boot:
    ...
    * If the store is `:empty` (never set at runtime), the COMPILE DEFAULT is
      ...
      `RacingOrg.Tracker.Pro.WiFiPower` boot behaviour); `compile_default == true` → leave the
