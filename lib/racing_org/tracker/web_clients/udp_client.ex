defmodule RacingOrg.Tracker.WebClients.UDPClient do
  @moduledoc """
  ...
  ONLY ever puts a sealed AEAD frame on the wire:
    ...
    * If a **live** SecureTransport session exists
      (`RacingOrg.Tracker.SecureTransport.SessionHolder.live?/1`), the DataSet binary is
  ...
  (`RacingOrg.Tracker.WebClients.UDPClient.Server`) is unchanged for legacy command
  coexistence; secure command delivery is the P4 channel.
  """

  require Logger

  alias RacingOrg.Tracker.SecureTransport.Frame
  alias RacingOrg.Tracker.SecureTransport.SessionHolder
  alias RacingOrg.Tracker.WebClients.UDPClient.Server
