defmodule RacingOrg.Tracker.Application do
  ...
  def start(_type, _args) do
    opts = [strategy: :one_for_one, name: RacingOrg.Tracker.Supervisor]
  ...
  defp start_virtual_device_and_handlers(sup) do
    {:ok, emit_telemetry_pid} =
      Supervisor.start_child(sup, {RacingOrg.Tracker.PacketHandler.EmitTelemetry, emit_telemetry_config()})

    {:ok, system_time_pid} = Supervisor.start_child(sup, RacingOrg.Tracker.PacketHandler.SetTimeFromGPS)
    {:ok, pid} = on_start = Supervisor.start_child(sup, {NMEA.NMEA2000.VirtualDevice, virtual_device_config()})

    # Expose the VirtualDevice so RacingOrg.Tracker.Nav.Broadcaster can transmit nav PGNs.
    RacingOrg.Tracker.put_virtual_device(pid)
  ...
  defp start_discovery(supervisor, virtual_device_pid) do
    {:ok, _discovery_pid} =
      Supervisor.start_child(supervisor, {RacingOrg.Tracker.Discovery, %{virtual_device_pid: virtual_device_pid}})
  end
  ...
  defp children(:logger, target) do
    [
      commands_child(),
      RacingOrg.Tracker.Telemetry,
      tracking_config_child(),
      compute_engine_child(),
      {RacingOrg.Tracker.Sampling, name: RacingOrg.Tracker.Sampling},
      archive_child(),
      {RacingOrg.Tracker.Nav.Broadcaster, name: RacingOrg.Tracker.Nav.Broadcaster},
      {RacingOrg.Tracker.Compute.Broadcaster, name: RacingOrg.Tracker.Compute.Broadcaster},
      race_timer_broadcaster_child(),
      waypoint_broadcaster_child(),
      {RacingOrg.Tracker.Serial, serial_config()},
      ...
      RacingOrg.Tracker.SecureTransport.SessionHolder,
      {RacingOrg.Tracker.WebClients.UDPClient, udp_config()},
      {RacingOrg.Tracker.DataSetRecorder, chunk_every: @max_unfragmented_udp_payload_size},
      {RacingOrg.Tracker.DataSetUploader, via: :udp}
  ...
  defp children(:uplink, target) do
    [
      ...
      {RacingOrg.Tracker.DataSetRecorder, chunk_every: @max_unfragmented_udp_payload_size},
      {RacingOrg.Tracker.DataSetUploader, via: :udp},
      RacingOrg.Tracker.BaseStation
    ] ++ wifi_manager_children(target) ++ secure_transport_children(target)
  end

  # Wi-Fi: on a real device target, `RacingOrg.Tracker.WiFiManager` OWNS wlan0 at runtime.
  ...
  # battery save). It reuses `RacingOrg.Tracker.WiFiPower` for the rfkill block/unblock and
  ...
  defp wifi_manager_children(target) do
    if real_target?(target) do
      [{RacingOrg.Tracker.WiFiManager, compile_default: wifi_enabled?()}]
    else
      []
    end
  end
  ...
  defp secure_transport_children(target) do
    if secure_transport_configured?(target) do
      [
        RacingOrg.Tracker.SecureTransport.BootProvisioner,
        RacingOrg.Tracker.SecureTransport.ChannelClient,
        RacingOrg.Tracker.Race.BulkUploader
      ]
    else
      []
    end
  ...
  def secure_transport_configured?(target) do
    real_target?(target) and RacingOrg.Tracker.SecureTransport.ServerIdentity.configured?()
  end
  ...
  defp commands_child do
    {RacingOrg.Tracker.Commands,
     name: RacingOrg.Tracker.Commands,
     device_id: RacingOrg.Tracker.boat_identifier(),
  ...
  # on apply, re-drives RacingOrg.Tracker.Sampling (which sets the Reporter flush interval +
  ...
  defp tracking_config_child do
    {RacingOrg.Tracker.Tracking.Config,
     name: RacingOrg.Tracker.Tracking.Config,
     store_dir: Application.get_env(:racing_org_tracker, :tracking_directory),
     on_apply: fn _config ->
       try do
         RacingOrg.Tracker.Sampling.reconfigure(RacingOrg.Tracker.Sampling)
       catch
         :exit, _ -> :ok
       end
     end}
  ...
  defp compute_engine_child do
    {RacingOrg.Tracker.Compute.Engine,
     name: RacingOrg.Tracker.Compute.Engine, store_dir: Application.get_env(:racing_org_tracker, :computed_values_directory)}
  end

  # Durable local race archiving + reconciliation with RacingOrg.
  defp archive_child do
    {RacingOrg.Tracker.Race.Archive,
     name: RacingOrg.Tracker.Race.Archive,
     base_dir: Application.get_env(:racing_org_tracker, :race_archive_directory),
     sampling: RacingOrg.Tracker.Sampling,
     device_id: RacingOrg.Tracker.boat_identifier()}
  end
  ...
  defp race_timer_broadcaster_child do
    {RacingOrg.Tracker.Compute.RaceTimerBroadcaster, name: RacingOrg.Tracker.Compute.RaceTimerBroadcaster}
  end
  ...
  defp waypoint_broadcaster_child do
    {RacingOrg.Tracker.Compute.WaypointBroadcaster, name: RacingOrg.Tracker.Compute.WaypointBroadcaster}
  end
  ...
  defp serial_config do
    Application.get_env(:racing_org_tracker, RacingOrg.Tracker.Serial, [])
  end
  ...
  def maybe_replay_log do
    if filename = Application.get_env(:racing_org_tracker, :replay_log) do
      RacingOrg.Tracker.DeviceCLI.replay_log(filename, realtime?: true)
    end
  end
end
