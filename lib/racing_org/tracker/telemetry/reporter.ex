defmodule RacingOrg.Tracker.Telemetry.Reporter do
  @moduledoc """
  Telemetry reporter
  ...
  in milliseconds. Used by `RacingOrg.Tracker.Sampling` to drive 1/5/10 Hz output.
  """
  ...
  `RacingOrg.Tracker.Sampling` to apply the active tracking state's `damping_seconds`.
  """
  ...
  def init(%{metrics: metrics, callback: callback, flush_interval_ms: flush_interval_ms} = init_args) do
    ...
    state = %{
      ...
      # RacingOrg.Tracker.Sampling via set_damping/2 from the active tracking state.
