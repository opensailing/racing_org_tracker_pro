defmodule RacingOrg.Tracker.WiFiManager do
  @moduledoc """
  ...
  The persisted state in `RacingOrg.Tracker.WiFi.Store` is the AUTHORITY. On boot:
    ...
    * If the store is `:empty` (never set at runtime), the COMPILE DEFAULT is
      ...
      `RacingOrg.Tracker.WiFiPower` boot behaviour); `compile_default == true` → leave the
  ...
  Started ONLY on a real device target (see `RacingOrg.Tracker.Application`); it decides
  ...
  VintageNet / `RacingOrg.Tracker.WiFiPower` implementations) so the reconcile and
  ...
  alias RacingOrg.Tracker.WiFi.Store
