defmodule RacingOrg.Tracker.SecureTransport do
  @moduledoc """
  ...
  **not** touch the database, channels, UDP, or protobuf; it operates only on binaries.
  ...
  The implementation lives in:
    ...
    * `RacingOrg.Tracker.SecureTransport.Primitives` — `:crypto` wrappers (Ed25519, X25519,
      ChaCha20-Poly1305 with the mandatory structural tag/nonce checks), X25519 point
      validation, constant-time compare.
    * `RacingOrg.Tracker.SecureTransport.HKDF` — RFC 5869 HKDF over HMAC-SHA256.
    * `RacingOrg.Tracker.SecureTransport.Handshake` — initiator + responder roles.
    * `RacingOrg.Tracker.SecureTransport.Session` — established session state.
    * `RacingOrg.Tracker.SecureTransport.Frame` — wire framing, seal/open.
    * `RacingOrg.Tracker.SecureTransport.ReplayWindow` — per-(session,epoch) replay defence.
