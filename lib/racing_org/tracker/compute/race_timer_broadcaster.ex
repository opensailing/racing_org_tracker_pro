defmodule RacingOrg.Tracker.Compute.RaceTimerBroadcaster do
  @moduledoc """
  ...
  When the device holds an active race assignment (`RacingOrg.Tracker.Commands`) carrying a
  gun time (`official_start_time`), each tick computes `gun - now`, encodes the B&G
  "Race Timer" message (PGN 130824, Key 117 — see
  `RacingOrg.Tracker.Compute.PgnEncode.race_timer_from/2`), and transmits it as a fast-packet
  broadcast. The value counts DOWN to the gun, then UP (elapsed) after it.

  It mirrors `RacingOrg.Tracker.Nav.Broadcaster` / `RacingOrg.Tracker.Compute.Broadcaster`: a
  ...
  (that stays in `RacingOrg.Tracker.Sampling.Phase`).
  """
  use GenServer
