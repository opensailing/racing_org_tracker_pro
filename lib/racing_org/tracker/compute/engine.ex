defmodule RacingOrg.Tracker.Compute.Engine do
  @moduledoc """
  ...
  changes — evaluating BOTH free-form expressions (via `RacingOrg.Tracker.Compute.Expr`, a
  safe RPN stack machine) AND the shipped native calcs (`RacingOrg.Tracker.Compute.Library`:
  ...
  mirroring `RacingOrg.Tracker.Tracking.Config`/`Tracking.Store`:
  ...
  `RacingOrg.Tracker.PacketHandler.EmitTelemetry` emits). Values are stored in CATALOG UNITS:
  ...
  alias RacingOrg.Tracker.Compute.Expr
  alias RacingOrg.Tracker.Compute.Library
  alias RacingOrg.Tracker.Compute.Store
  ...
  stream — notably `bearing_to_mark` (degrees), which `RacingOrg.Tracker.Nav.Broadcaster`
