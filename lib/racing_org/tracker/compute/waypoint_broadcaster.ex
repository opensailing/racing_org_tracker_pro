defmodule RacingOrg.Tracker.Compute.WaypointBroadcaster do
  @moduledoc """
  ...
  When the device holds an active race assignment (`RacingOrg.Tracker.Commands`) whose course
  ...
  See `RacingOrg.Tracker.Compute.PgnEncode.navigation_data_129284/1` and `route_wp_129285/1`
  for the wire layouts.

  It mirrors `RacingOrg.Tracker.Compute.RaceTimerBroadcaster`: a GenServer ticking on a timer,
  ...
  the destination lat/lon. This is the SAME derivation `RacingOrg.Tracker.Nav.State` uses. The
  ...
  alias RacingOrg.Tracker.Nav.Geo
  ...
  # --- destination derivation (mirrors RacingOrg.Tracker.Nav.State) -------------------------
