defmodule RacingOrg.Tracker.Telemetry do
  @moduledoc """
  Defines metrics and starts the telemetry reporter.

  See `RacingOrg.Tracker.PacketHandler.EmitTelemetry` for where these metrics are emitted.
  """

  alias RacingOrg.Tracker.DataSetRecorder
  alias RacingOrg.Tracker.DeviceInfo
  alias RacingOrg.Protobuf

  def child_spec(_opts) do
    %{
      id: RacingOrg.Tracker.Telemetry,
      start:
        {RacingOrg.Tracker.Telemetry.Reporter, :start_link,
         [[name: RacingOrg.Tracker.Telemetry.Reporter, metrics: metrics(), callback: &report_metric/3]]}
    }
  end
  ...
  """
  @spec report_metric([atom], RacingOrg.Tracker.DeviceInfo.id(), term) :: term
