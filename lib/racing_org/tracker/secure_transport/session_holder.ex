defmodule RacingOrg.Tracker.SecureTransport.SessionHolder do
  @moduledoc """
  Shared, supervised holder for the CURRENT live secure-transport `Session`.

  The `RacingOrg.Tracker.SecureTransport.ChannelClient` runs the device→server handshake
  ...
  `RacingOrg.Tracker.SecureTransport.Frame`). Reusing a `(key, counter)` pair under one
