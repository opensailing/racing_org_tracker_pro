defmodule RacingOrg.Tracker.SecureTransport.Frame do
  @moduledoc """
  ...
  alias RacingOrg.Tracker.SecureTransport.{Primitives, ReplayWindow, Session}
  ...
  This is what the P9-job-4 UDP telemetry path uses. `RacingOrg.Tracker.SecureTransport.SessionHolder`
  ...
  def parse_header(
        <<"SRT1", ver, type, aead, session_id::binary-size(16), epoch::32, counter::64>>
      ) do
    cond do
  ...
  def encode_header(<<session_id::binary-size(16)>>, epoch, counter)
    ...
    <<ST.magic()::binary, ST.protocol_version(), ST.type_data(), ST.aead_chacha20_poly1305(),
      session_id::binary, epoch::32, counter::64>>
  end
