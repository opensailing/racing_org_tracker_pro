defmodule RacingOrg.Tracker.SecureTransport.ChannelHandler do
  @moduledoc """
  PURE protocol logic for the secure-transport command channel, extracted from the
  Slipstream transport (`RacingOrg.Tracker.SecureTransport.ChannelClient`) so it can be
  ...
  Three concerns, all side-effect-light and deterministic given their inputs:
    ...
    * `handle_command/3` — given a server `"command"` payload, decode the
      `ServerReply` protobuf, apply it through `RacingOrg.Tracker.Commands` (the SAME
  ...
  alias RacingOrg.Tracker.SecureTransport.Handshake
  ...
  applying it through `RacingOrg.Tracker.Commands` (the shared, idempotent command path).

  `command_server` is the `RacingOrg.Tracker.Commands` GenServer name/pid (defaults to
