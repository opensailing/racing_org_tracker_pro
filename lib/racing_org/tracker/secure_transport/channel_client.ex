defmodule RacingOrg.Tracker.SecureTransport.ChannelClient do
  @moduledoc """
  ...
  `RacingOrg.Tracker.SecureTransport.Handshake.initiator_init/2` (via the pure
  ...
  `RacingOrg.Tracker.SecureTransport.SessionHolder` (the shared holder job-4 reads for
  AEAD UDP telemetry).

  ## Commands

  On a `"command"` push the client decodes the `ServerReply` protobuf and applies
  it through `RacingOrg.Tracker.Commands` (the existing, idempotent command handler), then
  pushes the `"ack"` event the server expects. Duplicate commands are de-duped by
  `RacingOrg.Tracker.Commands` and still acked (idempotent).
  ...
  (`RacingOrg.Tracker.SecureTransport.Backoff`) — never a hot loop. A fresh handshake runs
  on every reconnect.
  ...
  alias RacingOrg.Tracker.SecureTransport.Backoff
  alias RacingOrg.Tracker.SecureTransport.ChannelHandler
  ...
  Options (all optional; sensible production defaults):
    ...
    * `:commands` — the `RacingOrg.Tracker.Commands` server (default `RacingOrg.Tracker.Commands`).
    ...
    * `:wifi` — the WiFi collaborator that applies config + reports status. Either a
      module (used as both module and GenServer name, default `RacingOrg.Tracker.WiFiManager`)
    ...
    * `:backoff` — `RacingOrg.Tracker.SecureTransport.Backoff` opts.
  """
  ...
  def init(opts) do
    state = %{
      opts: opts,
      commands: Keyword.get(opts, :commands, RacingOrg.Tracker.Commands),
      session_holder: Keyword.get(opts, :session_holder, SessionHolder),
      wifi: normalize_wifi(Keyword.get(opts, :wifi, RacingOrg.Tracker.WiFiManager)),
      # The per-state tracking config (damping + send-rate). `tracking` applies the
      # server-pushed config (default RacingOrg.Tracker.Tracking.Config); `tracking_status`
      # reports what is actually being applied (default RacingOrg.Tracker.Sampling). Both
      # are {module, server} pairs (a bare module is used as both module + name).
      tracking: normalize_collaborator(Keyword.get(opts, :tracking, RacingOrg.Tracker.Tracking.Config)),
      tracking_status: normalize_collaborator(Keyword.get(opts, :tracking_status, RacingOrg.Tracker.Sampling)),
      ...
      compute: normalize_collaborator(Keyword.get(opts, :compute, RacingOrg.Tracker.Compute.Engine)),
      ...
      # "computed_values_status". A {module, server} pair (bare module = both).
      compute_broadcaster:
        normalize_collaborator(Keyword.get(opts, :compute_broadcaster, RacingOrg.Tracker.Compute.Broadcaster)),
      firmware_validator: Keyword.get(opts, :firmware_validator, &RacingOrg.Tracker.FirmwareValidator.validate_on_connect/0),
  ...
  # RacingOrg.Tracker.Tracking.Config (versioned, idempotent), then report what the device is
  ...
  # RacingOrg.Tracker.Compute.Engine (versioned, idempotent), then report applied_version +
  ...
  # device is actually applying (from RacingOrg.Tracker.Sampling). Falls back to a minimal map
  ...
  defp registered?(opts) do
    RacingOrg.Tracker.SecureTransport.BootProvisioner.registered?(keystore_opts(opts))
  end
