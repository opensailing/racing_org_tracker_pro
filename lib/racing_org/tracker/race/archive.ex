defmodule RacingOrg.Tracker.Race.Archive do
  @moduledoc """
  ...
  `RacingOrg.Tracker.Race.Recording`, appends every sampled `DataSet` produced during the
  ...
  alias RacingOrg.Tracker.Race.Recording
  alias RacingOrg.Tracker.Race.Retention
  ...
  def init(opts) do
    ...
    if sampling = opts[:sampling], do: RacingOrg.Tracker.Sampling.subscribe(sampling, self())

    state = %{
      base_dir: opts[:base_dir],
      commands: commands,
      enqueue: opts[:enqueue_fn] || (&RacingOrg.Tracker.DataSetRecorder.enqueue_encoded/1),
  ...
  defp secure_transport_configured? do
    RacingOrg.Tracker.SecureTransport.ServerIdentity.configured?()
  end

  defp provisioned? do
    match?({:ok, _}, RacingOrg.Tracker.SecureTransport.KeyStore.load())
  end
  ...
  defp send_manifest(state, manifest) do
    RacingOrg.Tracker.data_set([], manifest: manifest) |> DataSet.encode() |> state.enqueue.()
  end
