defmodule RacingOrg.Tracker.Race.BulkUploader do
  @moduledoc """
  Post-race signed HTTPS BULK upload of a finished race recording to RacingOrg.

  After a race finishes, the device holds a finalized `RacingOrg.Tracker.Race.Recording`
  ...
  Ed25519 signed-request assertion (`RacingOrg.Tracker.SecureTransport.SignedRequest` over
  ...
  ## Resumability / idempotency / robustness
    ...
    * A single chunk upload failing (network/transport/non-2xx) is LOGGED and does
      ...
      (`RacingOrg.Tracker.SecureTransport.Backoff`).
    * The local recording is NEVER deleted here. It is deleted only when RacingOrg
      ...
      device command (handled by `RacingOrg.Tracker.Race.Archive`); this uploader returns
  ...
  Required options:
    ...
    * `:race_session_id` — the server race-session id the manifest reconciles
      ...
      `RaceAssignment` command — see `RacingOrg.Tracker.Commands.Assignment`).
  ...
  # bytes — see RacingOrg.Tracker.Race.Recording).
