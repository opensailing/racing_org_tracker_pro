defmodule RacingOrg.Tracker.Tracking.Config do
  @moduledoc """
  ...
  Mirrors `RacingOrg.Tracker.WiFiManager` exactly:

    * The persisted config in `RacingOrg.Tracker.Tracking.Store` is the AUTHORITY. On boot it
    ...
    * `apply_config/2` is idempotent on `version`: a `version` already applied (`<=`
      ...
      side effect (which re-drives `RacingOrg.Tracker.Sampling`), and returns `{:ok, config}`.
  ...
  alias RacingOrg.Tracker.Tracking.Store
