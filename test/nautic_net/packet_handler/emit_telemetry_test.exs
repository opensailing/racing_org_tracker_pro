defmodule NauticNet.PacketHandler.EmitTelemetryTest do
  # Not async: attaches global :telemetry handlers and asserts on emitted events.
  use ExUnit.Case

  alias NauticNet.PacketHandler.EmitTelemetry
  ...
  test "attitude data is emitted on the :attitude event path, not :heading" do
    ...
    :telemetry.attach_many(
      handler_id,
      [[:nautic_net, :attitude], [:nautic_net, :heading]],
    ...
    assert_receive {:telemetry, [:nautic_net, :attitude], %{rad: %{yaw: 0.1, pitch: 0.2, roll: 0.3}}}
    refute_receive {:telemetry, [:nautic_net, :heading], _}, 50
  end
end
