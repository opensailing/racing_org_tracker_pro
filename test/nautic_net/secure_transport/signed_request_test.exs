defmodule NauticNet.SecureTransport.SignedRequestTest do
  @moduledoc """
  ...
  (`SailRoute.SecureTransport.SignedRequest.canonical/5`) BYTE-FOR-BYTE, signs it
  ...
  the headers the server's `SailRouteWeb.Plugs.DeviceSignedRequest` reads, with the
  ...
  # SailRoute.SecureTransport.SignedRequest.canonical/5:
  #
  ...
  describe "headers/5" do
    test "emits exactly the header names + encodings the server plug reads", %{identity: id} do
      ...
      assert Map.keys(headers) |> Enum.sort() == [
               "x-sailroute-device-fingerprint",
               "x-sailroute-signature",
               "x-sailroute-timestamp"
             ]

      # Fingerprint: lowercase hex SHA-256(public_key) (== KeyStore.fingerprint/1).
      assert headers["x-sailroute-device-fingerprint"] == id.fingerprint
      assert headers["x-sailroute-device-fingerprint"] == KeyStore.fingerprint(id.public_key)

      # Timestamp: unix seconds as an integer string.
      assert headers["x-sailroute-timestamp"] == Integer.to_string(ts)
      ...
      assert {:ok, sig} = Base.decode64(headers["x-sailroute-signature"])
      assert byte_size(sig) == 64
      message = server_canonical("POST", path, body, ts, id.fingerprint)
      assert Primitives.ed25519_verify(id.public_key, message, sig)
    end
    ...
    test "the timestamp in the header equals the one baked into the signed assertion", %{identity: id} do
      ...
      header_ts = String.to_integer(headers["x-sailroute-timestamp"])
