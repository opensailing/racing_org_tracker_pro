defmodule NauticNet.ApplicationTest do
  ...
  describe "secure_transport_configured?/1 gate predicate" do
    setup do
      ...
      on_exit(fn ->
        case prev do
          nil -> Application.delete_env(:nautic_net_device, ServerIdentity)
          value -> Application.put_env(:nautic_net_device, ServerIdentity, value)
        end
      end)
    ...
    test "host target is never configured, even with a pinned key set" do
      Application.put_env(:nautic_net_device, ServerIdentity, public_key: :crypto.strong_rand_bytes(32))

      refute NauticNet.Application.secure_transport_configured?(:host)
      refute NauticNet.Application.secure_transport_configured?(:"")
      refute NauticNet.Application.secure_transport_configured?(nil)
    end

    test "a real target with NO pinned key is not configured" do
      Application.delete_env(:nautic_net_device, ServerIdentity)
      refute NauticNet.Application.secure_transport_configured?(:nautic_net_rpi3)
    end

    test "a real target WITH a pinned key is configured" do
      Application.put_env(:nautic_net_device, ServerIdentity, public_key: :crypto.strong_rand_bytes(32))

      assert NauticNet.Application.secure_transport_configured?(:nautic_net_rpi3)
    end
  end
