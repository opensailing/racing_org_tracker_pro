defmodule RacingOrg.Tracker.Telemetry.ReporterTest do
  use ExUnit.Case
  ...
  describe "set_flush_interval/2" do
    test "changes the output flush rate at runtime" do
      ...
      assert :ok = RacingOrg.Tracker.Telemetry.Reporter.set_flush_interval(pid, 10)
  ...
  describe "set_damping/2 EWMA smoothing of signal measurements" do
    ...
    test "tau = 0 is pass-through: the last sample is reported verbatim (linear)" do
      ...
      assert :ok = RacingOrg.Tracker.Telemetry.Reporter.set_damping(pid, 0.0)
    ...
    test "a step input on a scalar field converges toward, but lags, the new value" do
      ...
      assert :ok = RacingOrg.Tracker.Telemetry.Reporter.set_damping(pid, 1.0)
  ...
  defp start_reporter(metrics, opts \\ []) do
    ...
    {:ok, pid} = RacingOrg.Tracker.Telemetry.Reporter.start_link(Keyword.merge(base, opts))
    pid
  end
